...
**Fase 1 MVP: FECHADA. Bloco 1 da Fase 2 (Transcendentais + Percentagens): CÓDIGO ESCRITO, EXECUÇÃO PENDENTE.** O **passo 10** concretizou os 13 eventos novos, implementou `reduceTranscendental`/`reducePercent` em `DefaultEngine`, destravou `Hp12cDecimal.sqrt()` real em `jvmCommonMain`, adicionou `Hp12cError.FactorialDomain` e escreveu a nova suíte `TranscendentalsVectorsTest` com 34 `@Test` individuais. Nenhuma linha da Fase 1 foi renomeada — API puramente aditiva.

**Débito imediato (bloqueio Cowork sandbox)**: `./gradlew :shared:jvmTest` **não rodou** — proxy bloqueia `services.gradle.org` (HTTP 403 no CONNECT), o wrapper não consegue baixar Gradle 9.3.1. Primeira ação em máquina dev:

- `./gradlew :shared:jvmTest` em ambiente com rede aberta — confirmar que a suíte sobe de 185 para ≈219 casos verdes.
- Se algum dos 34 vetores falhar, abrir passo 10.1 dedicado ao fix; não iniciar Bloco 2 antes disso.
- Vetores-bandeira a inspecionar primeiro se algo quebrar: `trans-fact-003` (`0!=1`), `trans-frac-002` (`FRAC(-3.88)=-0.88`), `trans-pct-retain-002` (Y = 300 após `%`).

**Próximo passo sugerido — Passo 11: Bloco 2 (Estatística Σ) — paperwork**. Seguindo o padrão `passo-paperwork → passo-código` que a skill (§"Fluxo de trabalho recomendado") recomenda e que o par passo 9 → passo 10 validou na prática. Entregáveis do passo 11:

1. **Novo `formulas/estatistica.md` em `.claude/skills/hp12c-simulator/formulas/`** cobrindo as 7 teclas estatísticas da HP 12C — **manual Seção 6 (p. 79-84)** para uso observável + **Apêndice E (p. 204-205)** para fórmulas fechadas + **Apêndice D (p. 194)** para condições de Error 2:
   - `Σ+` (`Σ+`) — acumula em R1..R6 (n, Σx, Σx², Σy, Σy², Σxy) e empurra `n` em X.
   - `Σ-` — subtrai ponto (corrige entrada errada); sem error code dedicado.
   - `g x̄` — média dos x em X, média dos y em Y.
   - `g s` — desvio-padrão amostral de x em X, de y em Y; população via trick documentado `g x̄ Σ+ g s` (manual p. 82 nota de rodapé 20).
   - `g ŷ,r` — estimativa de y em X e correlação r em Y (dado x).
   - `g x̂,r` — simétrico (dado y).
   - `g x̄w` — média ponderada (Σwx/Σw).
   - Fórmulas fechadas tiradas do Apêndice E p. 204-205: média, média ponderada, regressão linear (B = [Σxy − (Σx·Σy)/n] / [Σx² − (Σx)²/n], A = ȳ − Bx̄, ŷ = A+Bx, x̂ = (y−A)/B), correlação r, desvio-padrão amostral via `sₓ = √{[nΣx² − (Σx)²]/[n(n−1)]}`.
   - **Idiossincrasias a investigar e catalogar**: (a) `Σ+`/`Σ-` empurram `n` em X (manual p. 80-81 confirmado); (b) `Σ-` sem ponto prévio não lança erro — simplesmente decrementa R1 e subtrai das somas, podendo deixar estado inconsistente para `s`; (c) `f CLEAR Σ` zera R1-R6 e pilha (X=0,00 na sequência); (d) **compartilhamento de R1-R6 com STO/RCL do usuário** — R1..R6 são os *mesmos* slots de memória da Fase 1; usar `STO 3` durante modo estatístico sobrescreve Σx² e corrompe `s`/`ŷ,r`/`x̂,r`. A skill manda seguir o manual; registrar decisão em `formulas/estatistica.md` §1.

2. **Novo `test-vectors/estatistica-vectors.json`** — schema adaptado novamente:
   - Para `Σ+`/`Σ-`: `operations: [{kind: "sigma_plus", x: 1, y: 2}, ...]` em sequência, depois `query: "mean"|"stddev"|"regression"|"correlation"` e `expected_x`/`expected_y` (duas saídas).
   - Para `x̄`/`s`: lido após sequência de `Σ+`, compara ambos X e Y do estado final.
   - Distribuição alvo: ~20-30 vetores, idealmente 2/3 do manual + 1/3 Moretti. Caminho de erro: `x̄`/`s` com `n=0` ou `n=1` (Error 2 — "erro estatístico", Apêndice D p. 195).

3. **Sync JSON → `commonTest/resources/`** e **`SKILL.md` atualizada** (bullet nova em `formulas/`, bullet nova em `test-vectors/`) — mesma coreografia dos passos 9/10.

4. **Placeholder em `Event.kt`** — comentário `Event.Statistics.{SigmaPlus, SigmaMinus, Mean, StdDev, LinReg, ...}` antecipando os nomes dos eventos do passo 12. Não concretizar ainda.

5. **Decisão explícita sobre compartilhamento de memória** (R1-R6): o `MemoryRegisters` da Fase 1 aloca R0-R9+Ri. O manual diz que `Σ+` usa R1-R6. Temos duas opções: (a) compartilhar fisicamente os mesmos slots, replicando o risco do usuário HP de "esquecer o modo estatístico e sobrescrever R3 com STO 3"; (b) alocar registradores dedicados `StatisticsRegisters` separados. A skill manda seguir o manual — opção (a), com teste que valida colisão. Registrar decisão em `formulas/estatistica.md` §N.

**Estimativa de tamanho do passo 11**: ~200 linhas de markdown em `formulas/estatistica.md` + ~400 linhas de JSON em `estatistica-vectors.json` + ~30 linhas de update em `SKILL.md` + ~5 linhas de placeholder em `Event.kt`. Zero linha de engine nova. Suíte após passo 11: ≈219 casos (estado do passo 10 validado).

**Estimativa do par completo 11+12 (paperwork+código)**: ~3 sessões. O passo 12 herda o mesmo caminho crítico do passo 10 — concretizar `Event.Statistics.*`, escrever `reduceStatistics` em `DefaultEngine`, adicionar estatísticas em `MemoryRegisters` com compartilhamento R1-R6, novo teste `StatisticsVectorsTest` com os ~25 vetores.

**Checklist antes do passo 11 começar**:

- `./gradlew :shared:jvmTest` passando com **≈219 casos** verdes (fechamento real do passo 10 em máquina dev).
- Leitura integral de `formulas/transcendentais.md` + `referencias/bcd-rounding.md` — o passo 11 vai espelhar o formato de ambos.
- Abrir `bpia5314.pdf` nas p. 129-142 e `livromfhp12c.pdf` (FURG) nos capítulos de estatística para extrair exemplos canônicos.

**Blocos 3-8 seguem planejados** (calendário, NPV/IRR, depreciação, amortização, ALG, programação). Cada um repete o par paperwork + código conforme `§"Fluxo de trabalho recomendado"` da `SKILL.md` — cara no começo, barata a cada feature subsequente.
